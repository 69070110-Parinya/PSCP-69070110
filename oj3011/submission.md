...
```text
ข้อนี้ต้องการ ให้ผสมสี โดยจะมีแม่สีให้ 3 สี (Primary Color) คือ สีแดง (Red) สีเหลือง (Yellow) และสีน้ำเงิน (Blue)
...
แต่ว่านอกจากนี้จะตอบ Error
input
สีทั้ง 2 สี
output
สีที่ผสมได้ หรือ ตอบ Error
```
...
```text
ขั้นตอนแรก : คิดว่าข้อนี้ยังไงก็ต้องใช้ if แน่นอน วางแผนไว้เลยว่ารับมา 2 input แล้วก็ใช้ if ตรวจสอบเงื่อนไข
ต่อมา : ทดลองใช้ เงื่อนไขตามที่คิด
สุดท้ายตรวจสอบผล
```
...
```text
สุดท้าย ผิดบาง testcase เพราะใส่เงื่อนไขไม่ครบ จึงต้องกลับไปอ่านโจทย์และคิดอีกรอบ จนสุดท้ายก็สามารถผ่านได้
```
