...
```text
โจทย์ต้องการหาจำนวนคนที่ได้จับของขวัญ โดยเริ่มจากคนที่ 1 และส่งต่อทีละ K คนแบบวนเป็นวงกลม จนกว่าของขวัญจะส่งกลับมาถึงคนที่ 1 หรือตกไปอยู่ในมือของขโมย (คนที่ T)
Input: บรรทัดเดียว มีจำนวนเต็ม 3 จำนวนเรียงกัน คือ N (จำนวนคนทั้งหมด), K (ระยะการส่งต่อ) และ T (หมายเลขคนที่เป็นขโมย)
Output: บรรทัดเดียว แสดงจำนวนคนทั้งหมดที่ได้พิจารณาของขวัญ (รวมคนที่ 1 และขโมย)
```
...
```text
อ่านค่า N, K, T แล้วสร้างเซต six ไว้เก็บคนที่เคยได้พิจารณาของขวัญ โดยเริ่มให้คนที่ 1 ได้พิจารณาก่อน
ขั้นตอนการจำลอง: วนลูปบวกตำแหน่งด้วย K แล้วใช้ % N เมื่อเกิน N โดยถ้าเจอคนที่ไม่เคยพิจารณาจะใส่ลงในเซตและเพิ่มตัวนับ gift_check
เงื่อนไขการหยุด: ทำงานไปเรื่อย ๆ จนกว่าตำแหน่งปัจจุบัน checker จะเท่ากับขโมย T หรือวนกลับมาเจอคนที่ 1 (a_check มีค่าเป็น 1) แล้วพิมพ์ gift_check
```
...
```text
ใช้แบบวิธีเดิม เพราะเป็นวิธีที่ง่าย และไม่ซับซ้อน
```
...
```text
 เริ่มที่คนที่ 1 ซึ่งเจอขโมย T=3 ลูปจึงหยุดทำงานทันทีเพราะ checker == t
```
...
```text
เริ่มที่ 1 ขยับทีละ 1 ค่าไม่เกิน N=4 จึงไม่ต้องผ่านการมอดุโล และหยุดเมื่อเจอ T=4
```
