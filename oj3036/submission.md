...
```text
โจทย์ให้หาจำนวนกำแพงที่น้อยที่สุดที่ต้องพังเพื่อเดินทางจากห้อง N ไปยังห้อง 1 บนแผนที่พีระมิดสามเหลี่ยม โดยสามารถคิดแก้ปัญหาได้จากการหาแถว และหาลำดับคอลัมน์ของห้องนั้น เมื่อรู้พิกัดแล้ว ระยะทางที่สั้นที่สุดจะเกิดจาก ผลรวมของการขยับขึ้นชั้นทางแนวตั้ง บวกกับระยะห่างแนวนอน จากแกนกลางพีระมิด
input - จำนวนเต็มหนึ่งจำนวน
output - จำนวนกำแพงน้อยที่สุดที่จะต้องพัง เพื่อเดินทางจากห้อง N ไปจนถึงห้องหมายเลข 1 
```
...
```text
เริ่มที่ ห้อง 1 โดยมีความกว้างแถวปัจจุบัน w = 1 และจำนวนกำแพงที่พังไปแล้ว cnt = 0 บันทึกหมายเลขห้องปัจจุบัน u ลงในรายการห้องที่เคยผ่านแล้ว (vis) เพื่อไม่ให้เดินย้อนกลับมาซ้ำ จากนั้นตรวจสอบทางเลือกในแต่ละก้าว โดย ถ้าผลรวมของหมายเลขห้องกับความกว้างแถวปัจจุบันเป็นเลขคู่ ให้พังกำแพงข้ามไปยังห้อง u + 2wพร้อมกับปรับความกว้างแถวเพิ่มขึ้นเป็น w + 1 ถ้าหมายเลขห้องปัจจุบันยังมากกว่าขอบซ้ายสุดของแถวนั้น ให้พังกำแพงขยับไปห้องทางซ้าย u - 1 ถ้าหมายเลขห้องปัจจุบันยังไม่เกินขอบขวาสุดของแถวนั้น ให้พังกำแพงขยับไปห้องทางขวา u + 1 
```
...
```text
วิธีแรกไม่ได้ผล เพราะต้องทำซ้ำเรื่อยๆจนห้องปัจจุบันเท่ากับ N จึงจะแสดงผล cnt ออกมาได้ถูกต้อง
```
