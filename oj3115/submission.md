...
```text
-สร้าง Array นับจำนวนร้าน: สร้าง Array ขนาด 1,441 ช่อง (รองรับนาที 0 ถึง 1440) โดยกำหนดค่าเริ่มต้นเป็น 0 ทุกช่อง
-อัปเดตช่วงเวลาเปิด-ปิด: เมื่ออ่านค่า start และ stop ของแต่ละร้าน ให้บวกเพิ่ม 1 ในทุกๆ ช่องของ Array ตั้งแต่นาทีที่ start จนถึง stop - 1
-ค้นหาและแสดงผล: อ่านค่าช่วงเวลาใน check แล้วนำค่านั้นมาเป็น Index เพื่อดึงจำนวนร้านที่เปิดอยู่ ณ นาทีนั้นออกมาพิมพ์ตอบคั่นด้วยช่องว่างได้ทันที
```
...
```text
อ่านค่าจำนวนร้านค้าและจำนวนครั้งที่ต้องการตรวจสอบ จากนั้นรับข้อมูลช่วงเวลาเปิดและปิดของทุกร้านมาเก็บไว้ในรูปแบบของ List คู่ลำดับ (start, stop) เมื่อเข้าสู่ขั้นตอนการประมวลผล โปรแกรมจะวนลูปตามเวลาที่ต้องการตรวจสอบใน target_time ทีละค่า โดยในแต่ละนาทีจะนำไปเปรียบเทียบกับช่วงเวลาเปิด-ปิดของทุกร้านค้า หากเวลานั้นอยู่ระหว่าง start <= t < stop จะทำการเพิ่มนับจำนวนร้านค้าขึ้นทีละหนึ่ง เมื่อตรวจสอบครบทุกร้านแล้วจึงบันทึกผลลัพธ์ลงใน List และแสดงผลลัพธ์ทั้งหมดออกมาคั่นด้วยช่องว่างในบรรทัดเดียวตามที่โจทย์กำหนด
```
...
```text
ไม่มีร้านไหนเปิดเลยตลอดช่วงเวลาที่ตรวจ
```
