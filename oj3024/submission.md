...
```text
ข้อนี้เป็นการหา ความเป็นไปได้ โดยที่ input รับค่าโหวตทั้งหมด โดยที่มี 3 คน โหวตได้คนละ 0-10 คะแนน
บรรทัดที่ 2 รับค่าสูงสุดของทั้ง 3 คน ถ้าคะแนนโหวดของคนที่ให้คะแนนสูงสุด และคะแนนต่ำสุดห่างกันเกินสองคะแนน จะต้องแสดง output ว่า Surprising หรือ ถ้าน้อยกว่า 2 คะแนน แสดงว่า Not surprising
```
...
```text
บรรทัดที่ 1: รับค่าคะแนนรวม (total) และคะแนนสูงสุด (max_score) แปลงเป็นจำนวนจริง (float)

บรรทัดที่ 2: หาผลรวมคะแนนของอีก 2 คนที่เหลือ (rest) โดยเอาคะแนนรวม ลบ คะแนนสูงสุดออก

บรรทัดที่ 3: ตรวจสอบว่า rest มีค่าน้อยกว่าเกณฑ์ขั้นต่ำที่จะไม่เกิด Surprising หรือไม่ ถ้า น้อยกว่า (แปลว่าต้องมีคนที่ได้คะแนนต่ำกว่า max_score - 2) ให้พิมพ์ "Surprising" ถ้าไม่น้อยกว่าให้พิมพ์ "Not surprising"
```
...
```text
ตามแผน เพราะ เป็นไปตามเงื่อนไขที่คิด
```
